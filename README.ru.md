...
**[Посмотреть вживую](https://demo.statedash.geracorp.org)** — работающий стенд
...
- Таблица устройств: имя (ARP → DHCP → обратный DNS → traffic top), текущие
  ...
  активности, суммарный трафик — считается с того момента, как Statedash
  впервые увидел хост: собственного пожизненного счётчика файрвол не отдаёт.
...
**Плитки «Файрвол»** над графиком: процессор, память, подкачка, температура,
буферы сети, состояния pf и самая заполненная файловая система — чтобы
загруженную машину было видно отдельно от загруженной сети, не открывая вторую
вкладку. Полоска каждой плитки идёт от зелёного к красному по мере роста
показателя, а вышедшее за обычные пределы значение говорит об этом ещё и
словами: цвет сам по себе годится не всем. Строка убирается кнопкой, как график.

Это единственная часть, которой нужна привилегия **Lobby: Dashboard**, и без неё
строки нет вовсе, а не пусто. Плитки, по которым железу нечего сообщить, тоже не
показываются: машина без настроенной подкачки или без датчиков температуры,
которые умеет читать OPNsense, не покажет ни ту, ни другую — вместо нуля,
который выглядел бы как показание. Загрузка процессора даётся одним числом на
машину, а не по ядрам: больше эндпоинт не отдаёт. Цифры сходятся с тем, что
OPNsense показывает на своём дашборде — буферы сети считаются как занятые
кластеры против максимума, ровно как у него.

### История

Всё, что выше, — последние четверть часа: они лежат в памяти и пропадают вместе
с контейнером. Этот раздел переживает его. Один выбор промежутка, от пяти минут
до месяца, и четыре графика, которые ему следуют: пропускная способность по
интерфейсам, собственные показатели файрвола, диск и одно устройство на выбор.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/statedash-history-ru-dark.png">
  <img alt="history" src="docs/statedash-history-ru.png">
</picture>

**Откуда данные.** Файл SQLite рядом с `settings.json` или там, куда указывает
`HISTORY_PATH`. Отсчёты складываются в вёдра по мере поступления и пишутся,
когда ведро закрывается, — то есть ничего не хранится с частотой опроса. Две
полки: точка в минуту держится неделю, точка в час — пять недель, а по каждому
устройству двое суток минут и те же пять недель часов. Какая полка отвечает,
решается по тому, что на ней есть за запрошенное окно, поэтому хранилище,
прожившее десять минут, всё равно нарисует месячный вид по тем точкам, которые у
него есть. Дальше месяца история не смотрит, а лишняя неделя нужна, чтобы
месячный вид был полным с обоих концов. Меньше 15 МБ на нагруженной сети.

**Среднее и пик.** Каждая точка несёт оба: час усреднения скрыл бы всплеск, ради
которого этот час и смотрят. Линия рисует среднее, подсказка называет пик.

**Ось одна на все четыре.** Это выбранный промежуток, а не размах данных каждого
графика, поэтому устройство, замеченное двадцать минут назад, не занимает ту же
ширину, что сутки трафика. Где истории нет, там пусто. Диск — исключение,
подтверждающее правило: заполняется он процентами в месяц, поэтому получает ось
по собственному размаху, а не от нуля, где месяц заполнения был бы прямой.

**Выбор устройства.** Пять недель на настоящем файрволе — это не список ваших
устройств: в одном таком оказалось 466 адресов при 19 живых хостах, остальное —
дальние концы соединений, прошедших однажды. Поэтому список ищет по имени и по
адресу, а VPN-пиров и то, что сейчас в сети, показывает выше остального.

Если писать некуда, хранилище выключает себя, раздел уходит из меню, а остальное
продолжает работать: файловая система только для чтения — это не ошибка.
...
Что файрвол отклонил и почему. Данные берутся из журнала файрвола, а не из
таблицы состояний — заблокированный пакет состояния не создаёт, — и одинаковые
события свёрнуты в группы: правило запрета по умолчанию пишет строку на каждый
залётный пакет, и сырой список прокручивается быстрее, чем читается.

Три вида, различаемые по содержимому записи, а не по названию поймавшего
правила:

- **Попытки соединения** — кто-то пытался открыть соединение и получил отказ.
  Та категория, на которую стоит смотреть.
- **Запоздалые пакеты** — TCP с чем угодно, кроме одиночного SYN: состояние
  соединения уже истекло, и сброс или отставший пакет пришёл в пустоту. Дело
  обычное и, как правило, безобидное.
- **Широковещательный шум** — на широковещательный или групповой адрес.
  Соседние сегменты болтают непрерывно, поэтому шум свёрнут по умолчанию.

Раздел появляется только при выданной привилегии. Без неё не будет ни ошибки,
ни пустой страницы — пункта просто нет в меню.

Учти, что Suricata, если она у тебя работает, сюда не попадает: она разбирает
содержимое пакетов, а не заголовки, работает рядом с pf, а не внутри него, и
пишет в собственный журнал.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/statedash-blocked-ru-dark.png">
  <img alt="blocked" src="docs/statedash-blocked-ru.png">
</picture>
...
браузере. Длинные списки разбиты на страницы по пятьдесят строк, при этом
сортировка и фильтр работают по всему списку, а не по показанной странице.
Есть тёмная тема, два языка, мобильная вёрстка (липкая первая
...
| Diagnostics: Logs: Firewall: Live View | раздел «Заблокировано» |
| Lobby: Dashboard | плитки «Файрвол» (процессор, память, диск, состояния pf) |

Последние пять опциональны: без них соответствующие данные просто не появятся.
...
| `STATES_SECONDS` | 10 | период опроса соединений, правил, WireGuard, блокировок и метрик самого файрвола |
...
| `HISTORY_PATH` | рядом с settings.json | файл долгой истории; если писать некуда — остаётся только живой вид |
| `MOCK_HOSTS` | 0 | размер выдуманной сети в тестовом режиме (0 — фиксированный набор) |
| `DEMO` | 0 | витрина: всё видно, менять нельзя |
...
- **Живой вид живёт в памяти** (≈15 минут для хостов, ≈75 минут для WireGuard)
  и обнуляется при перезапуске контейнера. Длинные промежутки берутся из
  хранилища, описанного ниже, а ему нужно куда писать: без тома остаётся только
  живой вид.
- **Разрыв соединения** прерывает поток здесь и сейчас, но не запрещает его —
  клиент обычно переподключается.
- **Отдаёт простой HTTP.** Шифрование — задача того, кто стоит впереди:
  ingress или обратный прокси, как отдаётся
  [стенд](https://demo.statedash.geracorp.org). Выставленное напрямую,
  приложение не шифрует ничего; для доступа издалека VPN надёжнее открытого
  порта.
