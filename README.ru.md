...
**Промежутки.** Список над графиком, от пяти минут до месяца. Все они читаются из
файла SQLite рядом с `settings.json`: точка в минуту хранится неделю, точка в
час — пять недель, а по каждому устройству двое суток минут и те же пять недель
часов. Какая полка отвечает, решается по тому, что на ней есть за запрошенное
окно, поэтому хранилище, прожившее десять минут, всё равно нарисует месячный вид
по тем точкам, которые у него есть. В одном ответе не больше тысячи точек на
ряд: график примерно такой ширины в пикселях, а остальное ушло бы по сети только
затем, чтобы быть выброшенным. Дальше месяца история не смотрит, а лишняя неделя нужна, чтобы месячный
вид был полным с обоих концов. Каждая точка несёт среднее за интервал и пик
внутри него: час усреднения скрыл бы всплеск, ради которого этот час и смотрят,
поэтому линия рисует среднее, а подсказка называет пик. Меньше 15 МБ на
нагруженной сети.
